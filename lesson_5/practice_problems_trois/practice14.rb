=begin

Problem: 
        Given this data structure write some code to return an array containing 
        the colors of the fruits and the sizes of the vegetables. The sizes 
        should be uppercase and the colors should be capitalized.

        - Data Structure: nested hash
        - Return a NEW array
        - Colors of the Fruits              +         SIZES of the VEGETABLES
        - colors should be Capitalized      +         sizes should be UPPERCASe

Example:
        [["Red", "Green"], "MEDIUM", ["Red", "Green"], ["Orange"], "LARGE"]

Data Structure: 
        Input:   HASH
        Output:  ARRAY

Algorithm

Iterate through the array 
Should I use map or create a new variable?
Should I used a conditional or a Case Statement 


=end

hsh = {
  'grape' => {type: 'fruit', colors: ['red', 'green'], size: 'small'},
  'carrot' => {type: 'vegetable', colors: ['orange'], size: 'medium'},
  'apple' => {type: 'fruit', colors: ['red', 'green'], size: 'medium'},
  'apricot' => {type: 'fruit', colors: ['orange'], size: 'medium'},
  'marrow' => {type: 'vegetable', colors: ['green'], size: 'large'},
}

p (hsh.map do |_, value|
      if value[:type] == 'fruit'
        value[:colors].map do |color|
          color.capitalize
        end 
      elsif value[:type] == 'vegetable'
        value[:size].upcase     
      end 
    end)  

